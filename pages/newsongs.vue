<template>
  <div class="min-h-screen py-20 px-4 md:px-50">
    <h1 class="text-white text-[4rem] font-black mb-20 text-left">Новое на Новом</h1>

    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      <div
        v-for="(song, index) in songs"
        :key="index"
        class="flex items-center gap-3 bg-[#2e0863] border-[2px] border-[#6c02e6] shadow-[0_0_10px_#6c02e6] rounded-[12px] p-5 hover:bg-[#480b9e] cursor-pointer relative group"
      ...
      >
        <div class="relative">
          <img
            ...
            :alt="song.title"
            class="w-[80px] h-[80px] rounded-[12px] object-cover shrink-0"
          />
          <div
            v-if="song.demo"
...
const fetchNewSongs = async () => {
  try {
    const res = await fetch(`${API_BASE}/items/new_songs?sort=-date_created&limit=16`);
    ...
    songs.value = songsData.filter(song => song); // На случай null-ов
  } catch (err) {
