<template>
  <div class="w-full py-10 mb-10">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-8">
      <!-- Левая колонка: позиции 1–10 -->
      <div class="flex flex-col gap-6">
        <div
          v-for="(song, index) in songs.slice(0, 10)"
          :key="index"
          class="group flex items-center gap-3 bg-white/30 backdrop-blur-md border border-white/20 shadow-[0_0_15px_rgba(255,255,255,0.1)] hover:shadow-[0_0_20px_rgba(255,255,255,0.25)] rounded-[12px] cursor-pointer transition-colors hover:bg-[#9e0b97a2] py-4 px-5"
      ...
      <!-- Правая колонка: позиции 11–20 -->
      <div class="flex flex-col gap-6">
        <div
          v-for="(song, index) in songs.slice(10, 20)"
          :key="index"
          class="group flex items-center gap-3 bg-white/30 backdrop-blur-md border border-white/20 shadow-[0_0_15px_rgba(255,255,255,0.1)] hover:shadow-[0_0_20px_rgba(255,255,255,0.25)] rounded-[12px] cursor-pointer transition-colors hover:bg-[#9e0b97a2] py-4 px-5"
          @click="toggleDemo(song.demo)"
          :class="{ 'shadow-[0_0_20px_#d400ff]': currentDemo === `${API_BASE}/assets/${song.demo}` }"
...
const fetchNewSongs = async () => {
  try {
    const res = await fetch(`${API_BASE}/items/top_20?sort=position`);
